...
def index(request):
    ...
    if request.method=='POST':
        ...
        if  status=='200':
            print('200','111111111111111')
            info=wx.show_mobile_login()
            return render(request, 'index.html', locals())
        elif status=='201':
            print('201', '111111111111111')
            img = "images/QR.png"
            return render(request, 'index.html', locals())
        else :
            print('202', '111111111111111')
            img = "images/QR.png"
            return render(request, 'index.html', locals())
    else:
        loginInfo=wx.login()
...
def index1(request):
    ...
    if request.method=='POST':
        uuid=request.POST.get('uuid')
        status = wx.status(uuid)['status']
        if status=='200':
            print(status)
            wx.show_mobile_login()
            return HttpResponse('微信登录成功')
        else:
            return HttpResponse('微信登录shibai')
    else:
        with open('itchat.txt', 'r') as f:
            uuid = f.read()
        if wx.status(uuid)['status'] != '200':
            uuid = wx.get_uuid()
        QR = wx.open_QR(uuid)
        if QR == 'ok':
            print('111111111')
...
class wx_login():
    ...
    def check_login(self,uuid):
        ...
        if status=='200':
            result = {'status': '200', 'statusDES': '登陆成功'}
            return result
        elif status=='201':
            result = {'status': '201', 'statusDES': '已扫描二维码,手机上登录'}
            return result
        else:
            result = {'status': '0', 'statusDES': '已生成二维码'}
            return result
    ...
    def login(self):
        ...
        print(status)
        if status == '200' :
            result = {'status': '200','statusDES':'登录成功', 'uuid': uuid}
            return result
        elif status == '201':
            result = {'status': '201', 'statusDES': '请扫描登录', 'uuid': uuid}
            return result
        else:
            ...
            QR = imgDIR + '/QR.png'
            make_qr_code(http, QR)
            result = {'status': '1', 'statusDES': '已生成二维码,请扫描登录', 'uuid': uuid}
            return result
